...
const uploadProductImages = multer({
  ...
  fileFilter(_req, file, callback) {
    if (!ALLOWED_MIME.has(file.mimetype)) return callback(createUploadError('Допустимы только JPEG, PNG и WebP'));
    return callback(null, true);
  },
}).array('images', MAX_IMAGES);

function validateUploadedImage(file) {
  if (!file || !Buffer.isBuffer(file.buffer) || !ALLOWED_MIME.has(file.mimetype)) throw createUploadError('Некорректный файл изображения');
