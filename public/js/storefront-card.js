(function () {
  ...
  function create(product) {
    ...
    const badges=document.createElement('div'); badges.className='product-badges'; if(product.isNew){const b=document.createElement('span'); b.textContent='Новинка'; badges.append(b);} if(product.inStock===false){const b=document.createElement('span'); b.className='sold-out'; b.textContent='Нет в наличии'; badges.append(b);} media.append(link,badges,heart);
    const details=document.createElement('div'); details.className='product-details'; const category=document.createElement('small'); category.className='catalog-card-category'; category.textContent=product.category||'';
    ...
    const more=document.createElement('a'); more.className='add-button choose-size-button'; more.href=link.href; more.textContent=product.inStock===false?'Подробнее':'Выбрать размер'; details.append(category,title,price,more); article.append(media,details); return article;
  }
